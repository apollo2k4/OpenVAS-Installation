...
apt install -y libglib2.0-dev libgpgme-dev libgnutls28-dev uuid-dev libssh-gcrypt-dev libhiredis-dev libxml2-dev libpcap-dev libnet1-dev libpaho-mqtt-dev libldap2-dev libradcli-dev

# gvmd dependenices
apt install -y libglib2.0-dev libgnutls28-dev libpq-dev postgresql-server-dev-16 libical-dev xsltproc rsync libbsd-dev libgpgme-dev
...
# ospd-openvas
apt install -y python3 python3-pip python3-setuptools python3-wrapt python3-cffi python3-psutil python3-lxml python3-defusedxml python3-paramiko python3-redis python3-gnupg python3-paho-mqtt

# openvas
apt install -y rust-all pkg-config libssl-dev
...
export GVM_LIBS_VERSION=$(curl -s "https://api.github.com/repos/greenbone/gvm-libs/releases/latest" | grep -Po '"tag_name": "v\K[0-9.]+')
export GVMD_VERSION=$(curl -s "https://api.github.com/repos/greenbone/gvmd/releases/latest" | grep -Po '"tag_name": "v\K[0-9.]+')
export PG_GVM_VERSION=$(curl -s "https://api.github.com/repos/greenbone/pg-gvm/releases/latest" | grep -Po '"tag_name": "v\K[0-9.]+')
export GSA_VERSION=$(curl -s "https://api.github.com/repos/greenbone/gsa/releases/latest" | grep -Po '"tag_name": "v\K[0-9.]+')
export GSAD_VERSION=$(curl -s "https://api.github.com/repos/greenbone/gsad/releases/latest" | grep -Po '"tag_name": "v\K[0-9.]+')
