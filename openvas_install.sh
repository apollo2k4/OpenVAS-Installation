...
apt install -y libldap2-dev libradcli-dev libpq-dev postgresql-server-dev-15 libical-dev xsltproc rsync libbsd-dev krb5-multidev
...
apt install -y python3 python3-pip python3-setuptools python3-packaging python3-wrapt python3-cffi python3-psutil python3-lxml python3-defusedxml python3-paramiko python3-gnupg python3-paho-mqtt python3-venv python3-impacket
...
if grep -Fxq "mqtt_server_uri = localhost:1883\ntable_driven_lsc = yes" /etc/openvas/openvas.conf
then
else
